...
if (window.location.href.includes('videoCall')) { // 用来区分是呼叫Demo还是双录Demo，两个demo是同一套代码
    ...
    class videoCallClass {
        constructor() {
            ...
            window.addEventListener("beforeunload", (e) => {
                window.__fib = function (n) { // 注册一个耗时计算
...
((win, videoCall) => {
    class loginClass {
        ...
        // 打开设置面板
        openSetBox() {
            this.renderData({ // 取存储的参数给表单赋值并渲染
                server: this.server,
        ...
        onSaveSetData() {
            const that = this;
            win.layuiForm.on('submit(settFormSubmit)', function (data) { // 监听layui确定按钮表单提交事件
        ...
        initSDK() {
            ...
            CRVideo_Init(initParam).then((res) => { // 初始化成功
        ...
        loginToServer(cookie) {
            ...
            if (this.authType === 0) { // SDK接口：密码鉴权登录
                ...
                win.CRVideo_Login(appID, md5(appSecret), this.nickname, this.userID, userAuthCode, cookie);
            } else { // SDK接口：Token鉴权登录
        ...
        loginSuccessCallback(userID, cookie) {
            ...
            if (videoCall.login.loginIdent === 1) { // 客户身份
    ...
    class queueClass {
        ...
        initQueRsltCallback(sdkErr, cookie) {
            if (sdkErr == 0) {
                ...
                if (cookie == 'reLogin') {
                    ...
                    if (!sessionInfo.callID) { // 当前会话已经不在了，就不再进房间了
        ...
        updateQueListData(queInfo, queStatus, serviceStatus) {
            ...
            if (!!queInfo) {
                ...
                if (videoCall.isArrayFn(queInfo)) { // 更新的是queInfo集合
                    queInfo.forEach(i => {
                        let isNew = true;
            ...
            // 更新完数据后渲染
            if (videoCall.login.loginIdent === 0) { // 坐席队列
                // 更新队列服务情况
                if (!!serviceStatus) {
        ...
        addSeatModulFn() {
            ...
            this.autoAssignUserHandler = (CRVideo_QueUser) => {
                ...
                videoCall.modalLayer('客户分配中', `系统为您分配客户：${CRVideo_QueUser.nickname}`, {
                    btns: ['接受', '拒绝'],
                    btn1Callback() {
    ...
    class callClass {
        ...
        initData() {
            ...
            if (win.layuiForm !== undefined) {
                win.layuiForm.val('isDNDForm', { // 复原免打扰开关
                    DNDSwitch: null
                })
            }
        }
        ...
        hangupCallSuccessCallback(callID, cookie) {
            ...
            videoCall.meetMgr.exitMeeting(); // 退出房间
            videoCall.meetMgr.destroyMeeting(videoCall.meetMgr.meetInfo.ID); // 销毁房间
            if (videoCall.call.callType === 2) { // 呼叫类型为队列呼叫
    ...
    class meetMgrClass {
        ...
        enterMeetingRsltCallback(sdkErr, cookie) {
            if (sdkErr == 0) {
                ...
                if (videoCall.login.loginIdent === 0) { // 坐席身份
                    videoCall.meeting.showMeetingPage('seat'); // 显示客户通话界面
                } else if (videoCall.login.loginIdent === 1) { // 客户身份
                ...
                if (!!cookie && cookie.includes('reEnter')) { // 掉线重进房间
        ...
        // 离开房间
        exitMeeting() {
            if (videoCall.svrMixerMgr.recordState === 2) { // 录制中
                win.CRVideo_StopSvrMixer(); // SDK接口：停止录制、直播推流
            }
            if (videoCall.screenShare.sharerID === videoCall.login.userID) { // 自己共享屏幕
...
((win, videoCall) => {
    class audioVideoClass {
        ...
        setMemberVideo(videoUIObj, UID, camID) {
            ...
            videoUIObj.setNickNameStyle({ // 设置该组件的昵称显示样式
                left: "10px",
                top: "10px",
        ...
        // 显示坐席（主叫）画面
        showCallerVideo() {
            let callerVideoUIObj = CRVideo_CreatVideoObj({ // SDK接口：创建视频UI组件
        ...
        // 显示客户（被叫）画面
        showBeCalledVideo() {
            let beCalledVideoUIObj = CRVideo_CreatVideoObj({ // SDK接口：创建视频UI组件
                poster: './image/be_closed.jpg',
                style: {
                    // objectFit: 'cover' //object-fit样式
                }
...
((win, videoCall) => {
    class svrMixerMgrClass {
        ...
        showRecordSet() {
            ...
            win.layuiForm.val("recordSetBox", { // layui表单赋值
                fps: frameRate,
        ...
        getRecordFileInfo(filePathName) {
            ...
            const appID = videoCall.login.appID === '默认APPID' || videoCall.login.appID === '' ? 'demo@cloudroom.com' : videoCall.login.appID;
            const appSecret = videoCall.login.appSecret === '默认Secret' || videoCall.login.appSecret === '' ? '123456' : videoCall.login.appSecret;
            // 接口1：
            // const url = `${win.location.protocol}//${videoCall.login.server}/CLOUDROOM-API/netDisk/query`;
            // const data = {
            //     RequestId: new Date().getTime(),
            //     UserName: appID,
            //     UserPswd: md5(appSecret),
            //     AppId: appID,
            ...
            //                 // const url = `https://${videoCall.login.server}/${fileInfo.downUrl.split('//')[1].split('/').slice(1).join('/')}`;
            //                 // fileInfo.downUrl = url;
            ...
            // 接口2：
            const url2 = `${win.location.protocol}//${videoCall.login.server}/api/netdisk/getFiles`;
            const data2 = {
                _data_: JSON.stringify({
                    RequestId: new Date().getTime(),
                    UserName: appID,
                    UserPswd: md5(appSecret),
                    fileName: filePathName,
                ...
                success: function (res) {
                    ...
                    if (res.RspCode == 0) {
                        if (res.fileList && res.fileList.length == 1) {
                            ...
                            console.log(fileInfo.downUrl);
                            fileInfo.downUrl = fileInfo.downUrl.replace('http://192.168.0.25', 'https://crlab.cloudroom.com:543');
                            videoCall.svrMixerMgr.recordFileInfoList.push(fileInfo);
                            $('.list-record ul').append(`
                            <li>
                                <div class="name">${videoCall.svrMixerMgr.recordFileInfoList.length}、${fileInfo.fileName}</div>
...
((win, videoCall) => {
    class screenShareClass {
        ...
        notifyScreenShareStartedHandler(sharerID) {
            ...
            this.screenShareObj = win.CRVideo_CreatScreenShareObj({ // SDK接口：创建屏幕共享UI显示组件
            ...
            this.screenShareObj.setNickNameStyle({ // 设置该组件的昵称显示样式
                left: "10px",
                top: "10px",
                backgroundColor: "rgba(0, 0, 0, .7)",
                padding: "0 10px",
            ...
            if (videoCall.svrMixerMgr.recordState === 2) { // 云端录制中
...
((win, videoCall) => {
    class mediaShareClass {
        ...
        onClickPlayBtn(dom, id, isPlay) {
            ...
            if (isPlay === 'play') { // 点击播放按钮
                if ($(dom).hasClass('play')) { // 未播放状态点播放按钮（有暂停和停止两种状态）
                    if ($(dom).hasClass('play-pause')) { // 目前是暂停状态
                ...
                } else if ($(dom).hasClass('pause')) { // 播放状态点击播放按钮
